      ...
      <table className="w-full text-sm min-w-[640px]">
        <thead>
          <tr className="border-b border-border/60" style={{ background: "hsl(222 35% 10%)" }}>
            {[
              "Investor", "Type", "Entry Date", "Capital", "Years", "Current Value", "MOIC", "IRR"
            ].map((h) => (
              <th key={h} className="text-left px-3 py-2.5 text-[10px] font-semibold text-muted-foreground uppercase tracking-widest first:pl-4 last:pr-4">
                {h}
              </th>
            ))}
          </tr>
            ...
            return (
              ...
              >
                ...
                <td className="px-3 py-3 text-muted-foreground text-xs">
                  {formatDate(inv.subscriptionDate)}
                </td>
                <td className="px-3 py-3 font-numeric font-semibold text-foreground">
                  {formatEur(inv.capitalEur)}
                </td>
                <td className="px-3 py-3 text-muted-foreground text-xs">
                  {inv.yearsElapsed.toFixed(2)}y
                </td>
                <td className="px-3 py-3 font-numeric font-semibold text-foreground">
                  {formatEur(inv.currentValueEur)}
                </td>
                <td className={`px-3 py-3 font-numeric font-bold ${moicColor}`}>
                  {formatMultiple(inv.moic)}
                </td>
                <td className={`px-3 py-3 pr-4 font-numeric font-bold ${irrColor}`}>
                  {formatPct(inv.irrAnnualized)}
                </td>
              </tr>
            );
          })}
