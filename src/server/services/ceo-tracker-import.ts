...
function buildManualSeedRows(
  ...
  if (rows.length === 0) {
    ...
    if (participations > 0) {
      rows.push({
        itemKey: "PRIVATE_PARTICIPATIONS",
        displayName: "Private Participations",
        itemType: "Non-Listed",
        subcategory: "Participation",
        value: participations,
        notes: "Seeded from approved tracker partecipazioni current-value formula",
        sortOrder: 10,
      });
    }
    if (loans > 0) {
      rows.push({
        itemKey: "PRIVATE_LOAN_PRINCIPAL",
        displayName: "Private Loan Principal",
        itemType: "Non-Listed",
        subcategory: "Private Loan",
        value: loans,
        notes: "Seeded from approved tracker prestiti outstanding formula",
        sortOrder: 20,
      });
    }
  }
...
function isParticipationSeed(row: ManualSeedRow): boolean {
  ...
  return row.itemType === "Non-Listed" && (
    ...
    row.itemKey.startsWith("PRIVATE_PARTICIPATION_") ||
...
function isPrivateLoanSeed(row: ManualSeedRow): boolean {
  ...
  return row.itemType === "Non-Listed" && (
    ...
    row.itemKey.startsWith("PRIVATE_LOAN_") ||
...
function resultFromSnapshot(
  ...
  return {
    ...
    message: skipped
      ? `CEO tracker bootstrap already exists as snapshot ${snapshotId}.`
      : `CEO tracker bootstrap imported and published as snapshot ${snapshotId}. ${payload.investorPerformance.length} investor profile(s) upserted.`,
  };
}
...
async function latestCeoBaseline(): Promise<{ id: number; payload: PortfolioSnapshot } | null> {
  ...
  const rows = await prisma.$queryRaw<Array<{ id: bigint; payload: PortfolioSnapshot }>>`
    ...
    WHERE publication_status = 'published'
      AND (
        payload->'overlaySources'->>'seedSource' = 'ceo_tracker_bootstrap'
        OR payload->'overlaySources'->>'source' = 'CEO tracker workbook'
      )
...
async function refreshExistingCeoBaseline(
  ...
  const refreshed: PortfolioSnapshot = {
    ...
    dataFreshness: {
      lastUploadAt: payload.dataFreshness?.lastUploadAt ?? payload.sourceRecordedAt ?? null,
    ...
    investorPerformance,
    sourceRecordReady: true,
    sourceRecordedAt: payload.sourceRecordedAt,
    overlaySources: {
      ...
      seedSource: "ceo_tracker_bootstrap",
    },
  };
  const prisma = getPrisma();
  ...
  const result = await prisma.$transaction(async (tx) => {
    ...
    if (activeKeys.length > 0) {
      await tx.$executeRaw`
        ...
        WHERE (
          item_key LIKE 'PRIVATE_PARTICIPATION_%'
          OR item_key LIKE 'PRIVATE_LOAN_%'
          OR item_key IN ('PRIVATE_PARTICIPATIONS', 'PRIVATE_LOAN_PRINCIPAL', 'CASH_OUTSIDE_DIRECTA')
        )
