...
function parseWorkbook(wb: XLSX.WorkBook): WorkbookData {
  ...
  if (partSheet) {
    ...
    for (const [key, row] of grouped) {
      ...
      manualItems.push({
        itemKey: `PRIVATE_PARTICIPATION_${key}`,
        displayName: row.displayName,
        itemType: "Non-Listed",
        subcategory: "Participation",
        value: row.value,
        notes: "Seeded from approved tracker sheet 06_Partecipazioni",
        sortOrder: order,
      });
      order += 10;
    }
  ...
  if (loanSheet) {
    ...
    for (const [key, row] of grouped) {
      if (row.value <= 0) continue;
      manualItems.push({
        itemKey: `PRIVATE_LOAN_${key}`,
        displayName: row.displayName,
        itemType: "Non-Listed",
        subcategory: "Private Loan",
        value: row.value,
        notes: "Seeded from approved tracker sheet 07_Prestiti",
        sortOrder: order,
      });
      order += 10;
    }
